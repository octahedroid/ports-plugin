...
async function logPortInfo(
  ...
  const portRedirectListener = new PortRedirectListener(
    parseInt(workspacePort.portNumber, 10),
    "localhost",
    port.portNumber
  );
  ...
  const resultShow = await theia.window.showInformationMessage(
    msg,
    ...redirectInteractions
  );

  console.log(
    `Port Info - | URL: ${workspacePort.url} | Port: ${workspacePort.portNumber} | Server: ${workspacePort.serverName} | Preview URL: ${workspacePort.previewUrl}`
  );
}
...
async function onOpenPort(port: Port): Promise<void> {
  ...
  if (
    port.interfaceListen !== LISTEN_ALL_IPV4 &&
    port.interfaceListen !== LISTEN_ALL_IPV6
  ) {
    ...
    await logPortInfo(port, desc, err);
    return;
  }

  // check now if the port is in workspace definition ?
  const matchingWorkspacePort = workspacePorts.find(
    (workspacePort) => workspacePort.portNumber === port.portNumber.toString()
  );
  ...
  console.info(
    `The port ${port.portNumber} is now listening on interface ${port.interfaceListen}`
  );
}
...
function onClosedPort(port: Port): void {
  ...
  // just log trace
  console.info(
    `The port ${port.portNumber} is no longer listening on interface ${port.interfaceListen}`
  );
}
...
export async function start(context: theia.PluginContext): Promise<void> {
  ...
  redirectPorts = workspacePorts.filter((port) =>
    port.serverName.startsWith(SERVER_REDIRECT_PATTERN)
  );

  // initiate excluded ports
  const excludedPortProperties: string[] = Object.keys(
    process.env
  ).filter((key) => key.startsWith(PORT_EXCLUDE_ENV_VAR_PREFIX));
  excludedPortProperties.forEach((key) => {
    const value = process.env[key]!.toLocaleLowerCase() || "";
    if (value !== "no" && value !== "false") {
      excludedPorts.push(
        parseInt(key.substring(PORT_EXCLUDE_ENV_VAR_PREFIX.length))
      );
    }
  });
